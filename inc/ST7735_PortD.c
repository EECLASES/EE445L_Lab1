...
#include <stdint.h>
#include <stdlib.h>
#include "ST7735_PortD.h"
#include "../inc/tm4c123gh6pm.h"

// 16 rows (0 to 15) and 21 characters (0 to 20)
// Requires (11 + size*size*6*8) bytes of transmission for each character
uint32_t StX_PortD=0; // position along the horizonal axis 0 to 20
uint32_t StY_PortD=0; // position along the vertical axis 0 to 15
uint16_t StTextColor_PortD = ST7735_YELLOW;
...
#define TFT_CS                  (*((volatile uint32_t *)0x40007008))
#define TFT_CS_LOW              0           // CS normally controlled by hardware
#define TFT_CS_HIGH             0x02
...
static enum initRFlags_PortD TabColor;
...
void static writedata(uint8_t c) {
  while((SSI3_SR_R&SSI_SR_TNF)==0){};   // wait until transmit FIFO not full
  DC = DC_DATA;
  SSI3_DR_R = c;                        // data out
}
...
// Outputs: None
// Notes: ...
void Delay1ms_PortD(uint32_t n){uint32_t volatile time;
  while(n){
...
void static commonInit(const uint8_t *cmdList) {
  ...
  // SSI3Fss is temporarily used as GPIO
  GPIO_PORTD_DIR_R |= 0xC2;             // make PD1,6,7 (PA3,6,7) out
  GPIO_PORTD_AFSEL_R &= ~0xC2;          // disable alt funct on PD1,6,7 (PA3,6,7)
  GPIO_PORTD_DEN_R |= 0xC2;             // enable digital I/O on PD1,6,7 (PA3,6,7)
                                        // configure PD1,6,7 (PA3,6,7) as GPIO
  GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0x00FFFF0F)+0x00000000;
  GPIO_PORTD_AMSEL_R &= ~0xC2;          // disable analog functionality on PD1,6,7 (PA3,6,7)
  TFT_CS = TFT_CS_LOW;
  RESET = RESET_HIGH;
  Delay1ms_PortD(500);
  RESET = RESET_LOW;
  ...
  SSI3_CC_R = (SSI3_CC_R&~SSI_CC_CS_M)+SSI_CC_CS_SYSPLL;
//                                        // clock divider for 3.125 MHz SSIClk (50 MHz PIOSC/16)
//  SSI3_CPSR_R = (SSI3_CPSR_R&~SSI_CPSR_CPSDVSR_M)+16;
...
void ST7735_InitB_PortD(void) {
  ...
  StTextColor_PortD = ST7735_YELLOW;
  ST7735_FillScreen_PortD(0);                 // set screen to black
}
...
void ST7735_InitR_PortD(enum initRFlags_PortD option) {
  commonInit(Rcmd1);
  if(option == INITR_GREENTAB_PortD) {
  ...
  if (option == INITR_BLACKTAB_PortD) {
...
void ST7735_FillScreen_PortD(uint16_t color) {
  ST7735_FillRect_PortD(0, 0, _width, _height, color);  // original
...
void ST7735_DrawBitmap_PortD(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
...
// Output: none
void ST7735_DrawCharS_PortD(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size){
    ...
    for (j = 0; j<8; j++) {
      if (line & 0x1) {
        if (size == 1) // default size
          ST7735_DrawPixel_PortD(x+i, y+j, textColor);
        else {  // big size
          ST7735_FillRect_PortD(x+(i*size), y+(j*size), size, size, textColor);
        }
      } else if (bgColor != textColor) {
        if (size == 1) // default size
          ST7735_DrawPixel_PortD(x+i, y+j, bgColor);
        else {  // big size
          ST7735_FillRect_PortD(x+i*size, y+j*size, size, size, bgColor);
        }
      }
      line >>= 1;
    }
...
//        size      number of pixels per character pixel (e.g. size==2 prints each pixel of font as 2x2 square)
// Output: none
void ST7735_DrawChar_PortD(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size){
...
uint32_t ST7735_DrawString_PortD(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS_PortD(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
    pt++;
    x = x+1;
...
char Message_PortD[12];
uint32_t Messageindex_PortD;

void fillmessage_PortD(uint32_t n){
// This function uses recursion to convert decimal number
//   of unspecified length as an ASCII string
  if(n >= 10){
    fillmessage_PortD(n/10);
    n = n%10;
  }
  Message_PortD[Messageindex_PortD] = (n+'0'); /* n is between 0 and 9 */
  if(Messageindex_PortD<11)Messageindex_PortD++;
}
//********ST7735_SetCursor*****************
...
void ST7735_OutUDec_PortD(uint32_t n){
  Messageindex_PortD = 0;
  fillmessage_PortD(n);
  Message_PortD[Messageindex_PortD] = 0; // terminate
  ST7735_DrawString_PortD(StX_PortD,StY_PortD,Message_PortD,StTextColor_PortD);
  StX_PortD = StX_PortD+Messageindex_PortD;
  if(StX_PortD>20){
    StX_PortD = 20;
    ST7735_DrawCharS_PortD(StX_PortD*6,StY_PortD*10,'*',ST7735_RED,ST7735_BLACK, 1);
  }
}
...
void ST7735_SetRotation_PortD(uint8_t m) {
  ...
  switch (Rotation) {
   case 0:
     if (TabColor == INITR_BLACKTAB_PortD) {
...
int32_t Ymax_PortD,Ymin_PortD,X_PortD;        // X goes from 0 to 127
int32_t Yrange_PortD; //YrangeDiv2;
...
void ST7735_PlotClear_PortD(int32_t ymin, int32_t ymax){
  ST7735_FillRect_PortD(0, 32, 128, 128, ST7735_Color565_PortD(228,228,228)); // light grey
  if(ymax>ymin){
    Ymax_PortD = ymax;
    Ymin_PortD = ymin;
    Yrange_PortD = ymax-ymin;
  } else{
    Ymax_PortD = ymin;
    Ymin_PortD = ymax;
    Yrange_PortD = ymax-ymin;
...
// Outputs: none
void ST7735_PlotPoint_PortD(int32_t y){int32_t j;
  if(y<Ymin_PortD) y=Ymin_PortD;
  if(y>Ymax_PortD) y=Ymax_PortD;
  // X goes from 0 to 127
  // j goes from 159 to 32
  // y=Ymax maps to j=32
  // y=Ymin maps to j=159
  j = 32+(127*(Ymax_PortD-y))/Yrange_PortD;
  if(j<32) j = 32;
  if(j>159) j = 159;
  ST7735_DrawPixel_PortD(X_PortD,   j,   ST7735_BLUE);
  ST7735_DrawPixel_PortD(X_PortD+1, j,   ST7735_BLUE);
  ST7735_DrawPixel_PortD(X_PortD,   j+1, ST7735_BLUE);
  ST7735_DrawPixel_PortD(X_PortD+1, j+1, ST7735_BLUE);
}
// *************** ST7735_PlotLine ********************
...
// Inputs: y is the y coordinate of the point plotted
// Outputs: none
int32_t lastj_PortD=0;
void ST7735_PlotLine_PortD(int32_t y){int32_t i,j;
  if(y<Ymin_PortD) y=Ymin_PortD;
  if(y>Ymax_PortD) y=Ymax_PortD;
  // X goes from 0 to 127
  // j goes from 159 to 32
  ...
  if(j < 32) j = 32;
  if(j > 159) j = 159;
  if(lastj_PortD < 32) lastj_PortD = j;
  if(lastj_PortD > 159) lastj_PortD = j;
  if(lastj_PortD < j){
    for(i = lastj_PortD+1; i<=j ; i++){
      ST7735_DrawPixel_PortD(X_PortD,   i,   ST7735_BLUE) ;
      ST7735_DrawPixel_PortD(X_PortD+1, i,   ST7735_BLUE) ;
    }
  }else if(lastj_PortD > j){
    for(i = j; i<lastj_PortD ; i++){
      ST7735_DrawPixel_PortD(X_PortD,   i,   ST7735_BLUE) ;
      ST7735_DrawPixel_PortD(X_PortD+1, i,   ST7735_BLUE) ;
    }
  }else{
...
// Outputs: none
void ST7735_PlotPoints_PortD(int32_t y1,int32_t y2){int32_t j;
  if(y1<Ymin_PortD) y1=Ymin_PortD;
  if(y1>Ymax_PortD) y1=Ymax_PortD;
  // X goes from 0 to 127
  // j goes from 159 to 32
  // y=Ymax maps to j=32
  // y=Ymin maps to j=159
  j = 32+(127*(Ymax_PortD-y1))/Yrange_PortD;
  if(j<32) j = 32;
  if(j>159) j = 159;
  ST7735_DrawPixel_PortD(X_PortD, j, ST7735_BLUE);
  if(y2<Ymin_PortD) y2=Ymin_PortD;
  if(y2>Ymax_PortD) y2=Ymax_PortD;
  j = 32+(127*(Ymax_PortD-y2))/Yrange_PortD;
  if(j<32) j = 32;
  if(j>159) j = 159;
  ST7735_DrawPixel_PortD(X_PortD, j, ST7735_BLACK);
}
// *************** ST7735_PlotBar ********************
...
int32_t j;
  ...
  // y=Ymin maps to j=159
  j = 32+(127*(Ymax_PortD-y))/Yrange_PortD;
  ST7735_DrawFastVLine_PortD(X_PortD, j, 159-j, ST7735_BLACK);

}

// full scaled defined as 3V
// Input is 0 to 511, 0 => 159 and 511 => 32
uint8_t const dBfs_PortD[512]={
...
void ST7735_PlotNextErase_PortD(void){
  ...
  ST7735_DrawFastVLine_PortD(X_PortD,32,128,ST7735_Color565_PortD(228,228,228));
}
...
void ST7735_OutChar_PortD(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    StY_PortD++; StX_PortD=0;
    if(StY_PortD>15){
      StY_PortD = 0;
    }
    ST7735_DrawString_PortD(0,StY_PortD,"                     ",StTextColor_PortD);
    return;
  }
  ST7735_DrawCharS_PortD(StX_PortD*6,StY_PortD*10,ch,StTextColor_PortD,ST7735_BLACK, 1);
  StX_PortD++;
  if(StX_PortD>20){
    StX_PortD = 20;
    ST7735_DrawCharS_PortD(StX_PortD*6,StY_PortD*10,'*',ST7735_RED,ST7735_BLACK, 1);
...
void ST7735_SetTextColor_PortD(uint16_t color){
  StTextColor_PortD = color;
}
// Print a character to ST7735 LCD.
int fputc_PortD(int ch, FILE *f){
  ST7735_OutChar_PortD(ch);
  return 1;
}
// No input from Nokia, always return data.
int fgetc_PortD (FILE *f){
  return 0;
}
...
// Clear display
void Output_Clear_PortD(void){ // Clears the display
  ST7735_FillScreen_PortD(0);    // set screen to black
}
// Turn off display (low power)
void Output_Off_PortD(void){   // Turns off the display
  Output_Clear_PortD();  // not implemented
}
// Turn on display
void Output_On_PortD(void){ // Turns on the display
  Output_Init_PortD();      // reinitialize
}
// set the color for future output
// Background color is fixed at black
// Input:  16-bit packed color
// Output: none
void Output_Color_PortD(uint32_t newColor){ // Set color of future output
  ST7735_SetTextColor_PortD(newColor);
}
...
// put min and max of x and y into global variables
int	MinY_PortD = 0;
int	MaxY_PortD = 0;
int	MinX_PortD = 0;
int	MaxX_PortD = 0;
...
*/
void ST7735_XYplotInit_PortD(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY){
	...
	ST7735_FillScreen_PortD(ST7735_BLACK); // erase digits from the screen
	ST7735_SetCursor_PortD(0,0);					 // reset cursor to original position
	ST7735_OutString_PortD(title);				 // write the title on the screen
	ST7735_PlotClear_PortD(minY,maxY);		 // plot the white square on the screen
}
...
void ST7735_XYplot_PortD(uint32_t num, int32_t bufX[], int32_t bufY[]){
	...
	for(int i=0;i < num;i++){
		...
		j = (127*(bufX[i]-MinX_PortD))/(MaxX_PortD-MinX_PortD);
		// y=MaxY maps to j=32
		// y=MinY maps to j=159
		k = 32+(127*(MaxY_PortD-bufY[i]))/(MaxY_PortD-MinY_PortD);
		
		// Draw 4 pixels in each position from the buffers
		ST7735_DrawPixel_PortD(j, k, ST7735_RED);
		ST7735_DrawPixel_PortD(j+1, k, ST7735_RED);
		ST7735_DrawPixel_PortD(j, k+1, ST7735_RED);
		ST7735_DrawPixel_PortD(j+1, k+1, ST7735_RED);
	}
}

// plotLine function that is used when dx is greater than dy
void plotLineX_PortD(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color){
	int32_t d = 0;
	int32_t dx = x2-x1;
	int32_t dy = y2-y1;
	int32_t plotY = 0;
	...
	for(int32_t plotX = x1; plotX < x2; plotX++){
		if(abs(dx) > abs(dy)){
			ST7735_DrawPixel_PortD(plotX, plotY, color);
			ST7735_DrawPixel_PortD(plotX, plotY+1, color);
		}else{
			ST7735_DrawPixel_PortD(plotX, plotY, color);
			ST7735_DrawPixel_PortD(plotX+1, plotY, color);
		}
		if(d > 0){
			if(dy < 0){
				plotY--;
...
void plotLineY_PortD(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color){
	int32_t d = 0;
	int32_t dx = x2-x1;
	int32_t dy = y2-y1;
	int32_t plotX = 0;
...
//				color 16-bit color, which can be produced by ST7735_Color565() 
// Output: none
void ST7735_Line_PortD(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color){

	if(abs(y2 - y1) < abs(x2 - x1)){ // if dx is greater than dy
		
		if(x1 < x2){  // if x1 is to the left of x2
			plotLineX_PortD(x1, y1, x2, y2, color);
		}else{
			plotLineX_PortD(x2, y2, x1, y1, color);
		}
	}else{
		
		if(y1 < y2){  // if y1 is below y2
			plotLineY_PortD(x1, y1, x2, y2, color);
		}else{
			plotLineY_PortD(x2, y2, x1, y1, color);
		}
	}
}
